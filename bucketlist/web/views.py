...
def listBucketlists(request):
    ...
    if request.method == 'GET':
        ...
        if 'edit' in request.GET:
            ...
            edit_list = requests.get(
                domain + '/api/bucketlists/{}/'.format(edit_id), headers=headers)
        ...
        if 'delete' in request.GET:
            ...
            delete_list = requests.delete(
                domain + '/api/bucketlists/{}/'.format(delete_id), headers=headers)
        # List all lists
        url = domain + '/api/bucketlists/'
        if 'search' in request.GET:
            search = request.GET.get('search')
            url = domain + '/api/bucketlists/?search={0}'.format(search)
        bucketlists = requests.get(url, headers=headers)
    ...
    if request.method == 'POST':
        ...
        bucketlists = requests.post(
            domain + '/api/bucketlists/', data, headers=headers)
    return redirect('/web/bucketlists/', )
...
def listItems(request, id):
    ...
    if request.method == 'POST':
        ...
        if 'update' in request.POST:
            ...
            data = bucketlists
            data['name'] = name
            update = requests.put(url, data, headers=headers)
            return redirect('/web/bucketlists/{}/'.format(id), )
        data = {"name": name}
        item = requests.post(
            domain + '/api/bucketlists/{}/items/'.format(id), data, headers=headers)
        bucketlists = requests.get(
            domain + '/api/bucketlists/{}/'.format(id), headers=headers)
        lists = bucketlists.json()
...
def editItems(request, id, item):
    ...
    if request.method == 'GET':
        if 'delete' in request.GET:
            delete_list = requests.delete(
                domain + '/api/bucketlists/{0}/items/{1}/'.format(id, item), headers=headers)
            return redirect('/web/bucketlists/')
        lists = requests.get(
            domain + '/api/bucketlists/{0}/items/{1}/'.format(id, item), headers=headers)
        lists = lists
    ...
    if request.method == 'POST':
        ...
        if 'update' in request.POST:
            ...
            data['done'] = done
            update = requests.put(
                domain + "/api/bucketlists/{0}/items/{1}/".format(id, item), data, headers=headers)
